...
export function SidePanel({
  ...
  return (
    <div
      className="fixed inset-0 z-[60] flex justify-start bg-slate-950/30 backdrop-blur-[1px]"
      onPointerDown={(event) => {
        if (event.target === event.currentTarget) onClose();
      }}
    >
      <aside className="flex h-full w-full max-w-3xl flex-col border-r border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-[#171717]">
