#!/usr/bin/env node
// Generates (or checks) the SDKWork Partner App SDK family
// (sdks/sdkwork-partner-app-sdk) from the authored app-api authority input.
// Mirrors tools/partner_sdk_generate.mjs for the user-facing app surface;
// app operations are authenticated with portal sessions (x-sdkwork-auth-mode)
// instead of backend permissions.
...
const SDK_FAMILY = "sdkwork-partner-app-sdk";
const SDK_OWNER = "sdkwork-partner";
const API_AUTHORITY = "sdkwork-partner-app-api";
const API_PREFIX = "/app/v3/api";
const workspaceRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const familyRoot = path.join(workspaceRoot, "sdks", SDK_FAMILY);
const sourcePath = path.join(workspaceRoot, "apis", "app-api", "partner", `${API_AUTHORITY}.openapi.json`);
...
function validate(openapi) {
  ...
  for (const { method, operation, operationPath } of operations) {
    ...
    if (!String(operation["x-sdkwork-auth-mode"] ?? "").trim()) throw new Error(`${operation.operationId} missing auth mode`);
  }
...
try {
  ...
  if (!checkMode) {
    const result = spawnSync("node", [generatorBin, "generate", "--input", sdkgenPath, "--output", generatedRoot, "--name", SDK_FAMILY, "--type", "app", "--language", "typescript", "--base-url", "http://127.0.0.1:8080", "--api-prefix", API_PREFIX, "--fixed-sdk-version", "0.1.0", "--sdk-root", familyRoot, "--sdk-name", SDK_FAMILY, "--package-name", "sdkwork-partner-app-sdk-generated-typescript", "--standard-profile", "sdkwork-v3"], { cwd: familyRoot, stdio: "inherit" });
  ...
  process.stdout.write(`[partner_app_sdk_generate] ${checkMode ? "check passed" : "generation completed"} (${operationCount} operations)\n`);
} catch (error) {
  process.stderr.write(`[partner_app_sdk_generate] ${error instanceof Error ? error.message : String(error)}\n`);
  process.exit(1);
}
