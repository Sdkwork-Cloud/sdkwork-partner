...
const SDK_FAMILY = "sdkwork-partner-backend-sdk";
const SDK_OWNER = "sdkwork-partner";
const API_AUTHORITY = "sdkwork-partner-backend-api";
const API_PREFIX = "/backend/v3/api";
const workspaceRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const familyRoot = path.join(workspaceRoot, "sdks", SDK_FAMILY);
const sourcePath = path.join(workspaceRoot, "apis", "backend-api", "partner", `${API_AUTHORITY}.openapi.json`);
...
try {
  ...
  if (!checkMode) {
    const result = spawnSync("node", [generatorBin, "generate", "--input", sdkgenPath, "--output", generatedRoot, "--name", SDK_FAMILY, "--type", "backend", "--language", "typescript", "--base-url", "http://127.0.0.1:8080", "--api-prefix", API_PREFIX, "--fixed-sdk-version", "0.1.0", "--sdk-root", familyRoot, "--sdk-name", SDK_FAMILY, "--package-name", "sdkwork-partner-backend-sdk-generated-typescript", "--standard-profile", "sdkwork-v3"], { cwd: familyRoot, stdio: "inherit" });
  ...
  process.stdout.write(`[partner_sdk_generate] ${checkMode ? "check passed" : "generation completed"} (${operationCount} operations)\n`);
} catch (error) {
  process.stderr.write(`[partner_sdk_generate] ${error instanceof Error ? error.message : String(error)}\n`);
  process.exit(1);
}
